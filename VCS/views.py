...
from .models import BotQuestion,Company,Job,Profile,Subscription,Course,JobApplication,Appointment,Interaction,SupportQuery,Notification
...
def chat_api(request):
    ...
    if not user.profile.is_pro:
        today_count = Conversation.objects.filter(user=user).count()
        if today_count >= 10:
            return JsonResponse({"error": "Free users can ask up to 10 questions per day."})

    faq_queryset = BotQuestion.objects.filter(
        question__icontains=question
    )
    if not user.profile.is_pro:
        faq_queryset = faq_queryset.filter(is_pro=False)

    if faq_queryset.exists():
        best_match = max(
            faq_queryset,
            key=lambda q: SequenceMatcher(None, question.lower(), q.question.lower()).ratio()
        )
    ...
    Conversation.objects.create(user=user, question=question, answer=answer)
...
def conversation_history(request):
    ...
    return render(request, "conversation_history.html", {"conversations": conversations})


def chatfaq(request):
    if not request.user.is_authenticated or not request.user.is_superuser:
        return redirect('/')  # or some "no permission" page

    questions = BotQuestion.objects.all()
    ...
    return render(request, "chatfaq.html", context)
...
def add_question(request):
    if request.method == "POST":
        ...
        BotQuestion.objects.create(
            company=company, category=category, question=question, answer=answer, is_pro=is_pro
        )
        return redirect("chatfaq")

@login_required
def edit_question(request, qid):
    question_obj = get_object_or_404(BotQuestion, id=qid)
    companies = Company.objects.all()
    ...
    if request.method == "POST":
        ...
        question_obj.company = get_object_or_404(Company, id=company_id)
        question_obj.is_pro = request.POST.get("is_pro") == "on"
        question_obj.save()
        return redirect("chatfaq")

    return render(request, "edit_question.html", {"question": question_obj, "companies": companies})

@login_required
def delete_question(request, qid):
    question_obj = get_object_or_404(BotQuestion, id=qid)
    question_obj.delete()
    return redirect("chatfaq.html")
